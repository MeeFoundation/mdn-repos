...
pub trait MeeAuthoritySignatureService {
...
#[derive(Clone)]
pub struct MeeAuthoritySignatureServiceDefault {
    secret_path: String,
    secrets_manager: Arc<dyn SecretsManagerClient + Send + Sync>,
}

impl MeeAuthoritySignatureServiceDefault {
    pub fn new(
        secret_path: String,
        secrets_manager: Arc<dyn SecretsManagerClient + Send + Sync>,
    ) -> Self {
...
#[async_trait::async_trait]
impl MeeAuthoritySignatureService for MeeAuthoritySignatureServiceDefault {
    async fn get_signature(&self) -> anyhow::Result<Option<Jwk>> {
        let Some(jwk) =
