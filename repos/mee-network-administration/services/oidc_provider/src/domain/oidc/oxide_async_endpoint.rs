...
use oxide_auth::{
    endpoint::{
        OAuthError, OwnerConsent, Scope, Scopes, Solicitation, Template,
        WebRequest,
    },
...
use oxide_auth_async::{
    endpoint::{
        access_token::AccessTokenFlow, authorization::AuthorizationFlow,
        refresh::RefreshFlow, resource::ResourceFlow, Endpoint, Extension,
        OwnerSolicitor,
    },
...
pub struct FnSolicitorWithPayload<F, P>(pub F, pub P);
...
impl<
    ...
    > Generic<R, A, I, O, C, L>
{
    pub fn with_solicitor<N: Send + Sync>(
        self,
        new_solicitor: N,
    ) -> Generic<R, A, I, N, C, L> {
        Generic {
    ...
    pub fn _with_scopes<S: Send + Sync>(
        self,
        new_scopes: S,
    ) -> Generic<R, A, I, O, S, L> {
        Generic {
    ...
    pub fn authorization_flow<W: WebRequest + Send + Sync>(
        self,
    ) -> AuthorizationFlow<Self, W>
    where
        Self: Endpoint<W>,
        W::Error: Send + Sync,
        R: Registrar,
    ...
    pub fn access_token_flow<W: WebRequest + Send + Sync>(
        self,
    ) -> AccessTokenFlow<Self, W>
    where
        Self: Endpoint<W>,
        W::Error: Send + Sync,
        R: Registrar,
    ...
    pub fn refresh_flow<W: WebRequest + Send + Sync>(
        self,
    ) -> RefreshFlow<Self, W>
    where
        Self: Endpoint<W>,
    ...
    pub fn _resource_flow<W: WebRequest + Send + Sync>(
        self,
    ) -> ResourceFlow<Self, W>
    where
        Self: Endpoint<W>,
    ...
    pub fn _assert<W: WebRequest>(self) -> Self
    where
        Self: Endpoint<W>,
    {
        self
    ...
    W: WebRequest,
{
    ...
    fn owner_solicitor(
        &mut self,
    ) -> Option<&mut (dyn OwnerSolicitor<W> + Send)> {
...
impl<W, F> oxide_auth::endpoint::OwnerSolicitor<W> for FnSolicitor<F>
where
    W: WebRequest,
...
impl<W, F, P> oxide_auth::endpoint::OwnerSolicitor<W>
    for FnSolicitorWithPayload<F, P>
where
    W: WebRequest,
    P: Clone,
    F: FnMut(&mut W, Solicitation, P) -> OwnerConsent<W::Response>,
{
    fn check_consent(
        &mut self,
        request: &mut W,
        ...
        (self.0)(request, solicitation, self.1.clone())
    }
}

impl<W: WebRequest> oxide_auth::endpoint::OwnerSolicitor<W> for Vacant {
    fn check_consent(
        &mut self,
        _: &mut W,
        _: Solicitation,
