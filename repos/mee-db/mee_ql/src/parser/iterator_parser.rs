    ...
    fn parse_source(
        ...
        match node.kind() {
            "path" => {
                ...
                let source = mee_node(
                    &node,
                    Expression::Link(mee_node(&node, path.value).with_type(NodeTypes::Users)),
                )
            ...
            "array" => {
                ...
                let mut source = mee_node(&node, Expression::Array(array.into_iter().collect()));
...
impl Parser<MeeNode<IteratorStmt>> for IteratorStmtParser {
    fn parse(
        ...
        let source = self.parse_source(source_text, source_node, parser_list, ctx)?;
