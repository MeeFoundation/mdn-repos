...
#[utoipa::path(
  ...
  responses(
      (status = 200, description = "Returns capabilities to read subset of user's data", body = Object),
      (status = 500, description = "Something went wrong", body = String),
  ),
)]
...
) -> AgentServiceResult<Json<MdnProviderCapabilityPack>> {
    let caps = app_ctl
        .mdn_provider_agent_ctl
        .mdn_provider_agent_node_service
...
#[utoipa::path(
  ...
  responses(
    (
        ...
        body = Option<GetPersonaAttributesResponse>,
    ),
    (status = 500, description = "Something went wrong", body = String),
  ),
)]
...
) -> AgentServiceResult<Json<Option<GetPersonaAttributesResponse>>> {
    let res = app_ctl
        .mdn_provider_agent_ctl
        .mdn_provider_agent_node_service
