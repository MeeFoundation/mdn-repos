...
gen-dev-secrets:
  #!/bin/sh
  set -eu
  cd repos/mee-network-administration/services/mee_dev_secrets
  make gen_dev_mee_signature

gen-demo-provider-secrets:
  #!/bin/sh
  set -eu
  cd repos/mee-network-provider/services/dev_secrets_keygen
  make gen_dev_provider_signature1
  make gen_dev_provider_signature2
